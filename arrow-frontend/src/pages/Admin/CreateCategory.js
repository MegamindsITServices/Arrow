  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://api.arrowpublications.in/api/v1/category/create-category",
        {
          name,
        }
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://api.arrowpublications.in/api/v1/category/update-category/${selected._id}`,
        { name: updatedName }
      );
      if (data.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `https://api.arrowpublications.in/api/v1/category/delete-category/${pId}`
      );
      if (data.success) {
