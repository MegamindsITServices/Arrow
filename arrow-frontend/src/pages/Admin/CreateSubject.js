  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const { data } = await axios.post(
        "https://api.arrowpublications.in/api/v1/subject/create-subject",
        {
          name,
        }
      );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://api.arrowpublications.in/api/v1/subject/update-subject/${selected._id}`,
        { name: updateName }
      );
      if (data.success) {
  ...
  const handleDelete = async (pId) => {
    try {
      const { data } = await axios.delete(
        `https://api.arrowpublications.in/api/v1/subject/delete-subject/${pId}`
      );
      if (data.success) {
