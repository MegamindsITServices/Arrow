    ...
    const getAllOwners = async () => {
      try {
        const res = await axios.get(
          "https://api.arrowpublications.in/api/v1/auth/get-owners"
        );
  ...
  const handleUpdate = async (e) => {
    ...
    try {
      const { data } = await axios.put(
        `https://api.arrowpublications.in/api/v1/auth/admin-profile-update/${selectedOwner._id}`,
        updateOwner
      );
      if (data.success) {
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axios.delete(
        `https://api.arrowpublications.in/api/v1/auth/delete-owner/${id}`
      );
      if (data.success) {
